...
export class OverViewComponent implements OnInit, OnDestroy {
    ...
    this.loadNextPage().then((edges) => {
      this.service.metadata
        ...
        .subscribe((metadata) => {
          ...
          this.loggedInUserCanInstall = Role.isAtLeast(
            metadata.user.globalRole,
            'installer'
          );
          ...
          if (
            ...
            (!this.loggedInUserCanInstall && edgeIds.length == 1)
          ) {
  ...
  loadNextPage(): Promise<Edge[]> {
    ...
    return new Promise<Edge[]>((resolve, reject) => {
      ...
      console.log(
        '🚀 ~ file: overview.component.ts:149 ~ OverViewComponent ~ loadNextPage ~ searchParamsObj:',
        searchParamsObj
      );

      this.service
        .getEdges(this.page, this.query, this.limit, searchParamsObj)
        .then((edges) => {
