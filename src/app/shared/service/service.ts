...
export class Service extends AbstractService {
  ...
  public isSmartphoneResolutionSubject: Subject<boolean> =
    new Subject<boolean>();
   ...
   */
  public readonly currentEdge: BehaviorSubject<Edge> =
    new BehaviorSubject<Edge>(null);
  ...
  public setCurrentComponent(
    currentPageTitle:
      | string
      | { languageKey: string; interpolateParams?: Object },
  ...
  public queryEnergy(
    ...
    let response = new Promise<QueryHistoricTimeseriesEnergyResponse>(
      (resolve, reject) => {
      ...
      this.queryEnergyTimeout = setTimeout(() => {
          ...
          if (mergedRequests.length == 0) {
            ...
            for (let mergedRequest of mergedRequests) {
              if (
                ...
                // same date -> merge
                mergedRequest.promises = mergedRequest.promises.concat(
                  request.promises
                );
        ...
        this.getCurrentEdge().then((edge) => {
          for (let source of mergedRequests) {
            ...
            let request = new QueryHistoricTimeseriesEnergyRequest(
  ...
  public getEdges(
    ...
    query?: string,
    limit?: number,
    searchParamsObj?: { [id: string]: ChosenFilter['value'] }
  ): Promise<Edge[]> {
    console.log(
      '🚀 ~ file: service.ts:351 ~ Service ~ searchParamsObj:',
      searchParamsObj
    );
    return new Promise<Edge[]>((resolve, reject) => {
      this.websocket
        ...
        .then((response) => {
          console.log(
            '🚀 ~ file: service.ts:296 ~ Service ~ getEdges ~ response:',
            response
          );
  ...
  public updateCurrentEdge(edgeId: string): Promise<Edge> {
    return new Promise<Edge>((resolve, reject) => {
      ...
      this.websocket
        .sendSafeRequest(new GetEdgeRequest({ edgeId: edgeId }))
        .then((response) => {
