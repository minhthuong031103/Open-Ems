...
export class Websocket implements WebsocketInterface {
      ...
      .subscribe(
        (originalMessage) => {
          ...
          let message:
            ...
            | JsonrpcNotification
            | JsonrpcResponseSuccess
            | JsonrpcResponseError = JsonrpcMessage.from(originalMessage);
          ...
          if (message instanceof JsonrpcRequest) {
            ...
            if (environment.debugMode) {
              if (
                message.method == EdgeRpcNotification.METHOD &&
    ...
    return new Promise<void>((resolve) => {
      this.sendRequest(request)
        .then((r) => {
          ...
          // let language = Language.getByKey(
          //   localStorage.DEMO_LANGUAGE ??
          //     authenticateResponse.user.language.toLocaleLowerCase()
          // );
          let language = Language.getByKey('en');
          console.log(
            '🚀 ~ file: websocket.ts:193 ~ Websocket ~ .then ~ language:',
            language,
            authenticateResponse.user.language.toLocaleLowerCase()
          );
          ...
          // this.service.setLang(Language.EN);
          this.status = 'online';

          // received login token -> save in cookie
          this.cookieService.set('token', authenticateResponse.token, {
            expires: 365, // 1 year
            path: '/',
            sameSite: 'Strict',
          });
          ...
          this.service.getCurrentEdge().then((edge) => {
            console.log(
              '🚀 ~ file: websocket.ts:214 ~ Websocket ~ this.service.getCurrentEdge ~ edge:',
              edge
            );
  ...
  private checkErrorCode(reason: JsonrpcResponseError) {
    ...
    switch (reason.error.code) {
      case 1003:
        this.service.toast(
          this.translate.instant('Login.authenticationFailed'),
          'danger'
        );
        this.onLoggedOut();
        break;
  ...
  public sendRequest(request: JsonrpcRequest): Promise<JsonrpcResponseSuccess> {
    if (
      ...
      return new Promise((resolve, reject) => {
        this.wsdata
          .sendRequest(this.socket, request)
          .then((response) => {
            if (environment.debugMode) {
