...
export class AppComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    service.setLang(
      Language.getByKey(localStorage.LANGUAGE) ??
        Language.getByBrowserLang(navigator.language)
    );
    ...
    this.service.metadata
      ...
      .subscribe((metadata) => {
        this.isUserAllowedToSeeOverview =
          UserPermission.isUserAllowedToSeeOverview(metadata.user);
      });
  }
  ...
  ngOnInit() {
    ...
    this.platform.ready().then(() => {
      ...
      const notchColor = getComputedStyle(
